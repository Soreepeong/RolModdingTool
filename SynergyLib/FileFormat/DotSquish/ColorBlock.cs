...
namespace SynergyLib.FileFormat.DotSquish {
    ...
    internal static class ColorBlock {
        ...
        public static void WriteColorBlock(int a, int b, ReadOnlySpan<byte> indices, Span<byte> bytes) {
            ...
            for (var i = 0; i < 4; ++i) {
                var ind = indices[(4 * i)..];
                bytes[4 + i] = unchecked((byte) (ind[0] | (ind[1] << 2) | (ind[2] << 4) | (ind[3] << 6)));
            }
        }

        public static void WriteColorBlock3(in Vector3 start, in Vector3 end, ReadOnlySpan<byte> indices, Span<byte> block) {
        ...
        public static void WriteColorBlock4(in Vector3 start, in Vector3 end, ReadOnlySpan<byte> indices, Span<byte> block) {
            // get the packed values
            var a = FloatTo565(start);
            var b = FloatTo565(end);
        ...
        private static int Unpack565(ReadOnlySpan<byte> packed, int packedOffset, Span<byte> color, int colorOffset) {
            ...
            // Scale up to 8 bits
            color[colorOffset + 2] = (byte) ((red << 3) | (red >> 2));
            color[colorOffset + 1] = (byte) ((green << 2) | (green >> 4));
            color[colorOffset + 0] = (byte) ((blue << 3) | (blue >> 2));
            color[colorOffset + 3] = 255;

            return value;
        }

        public static void DecompressColor(Span<byte> bgra, ReadOnlySpan<byte> block, bool isDxt1) {
